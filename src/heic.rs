...
pub async fn convert_heic(
    ...
    let mime_type = inferred.get(&heic_data).unwrap();
    let mime_type = mime_type.mime_type();
    println!("{}", mime_type);
    let jpeg_data: Vec<u8>;
    match mime_type {
        "image/heif" => {
            jpeg_data = decode_heic_to_jpeg(&heic_data, t_type)?;
        }
        _ => jpeg_data = decode_jpeg_to_jpeg(&heic_data, t_type)?,
    }
    Ok(HttpResponse::Ok()
...
use image::{DynamicImage, ImageBuffer, ImageError, ImageFormat, ImageReader, Rgb, Rgba};
use libheif_rs::{ColorSpace, HeifContext, LibHeif, RgbChroma};

fn decode_heic_to_jpeg(
    heic_data: &[u8],
    t_type: ImageFormat,
...
fn decode_jpeg_to_jpeg(
    ...
    let reader = ImageReader::new(Cursor::new(data))
        .with_guessed_format()?
        .decode()?;
    let output_data: Vec<u8> = match reader {
        DynamicImage::ImageRgb8(img) => {
            let mut buffer = Vec::new();
            img.write_to(&mut Cursor::new(&mut buffer), t_type)?;
            buffer
        }
